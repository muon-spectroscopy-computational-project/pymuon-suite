...
_positions = [
    [0.241132, 0.0, 0.708084],
    [-0.04628, -0.0, -0.747689],
    [-0.164565, 0.885014, 1.20288],
    [-0.164565, -0.885014, 1.20288],
    [-0.094178, 0.924421, -1.305393],
    [-0.094178, -0.924421, -1.305393],
    [1.322635, 0.0, 0.91211],
]
...
def _test_write(expected_params, input_file=None, calc=None, atoms_calc=None):
    ...
    try:
        ...
        atoms_read = io.read(
            os.path.join(out_folder, "test_gaussian.com"), attach_calculator=True
        )
        ...
        for key, value in expected_params.items():
            params_equal = expected_params.get(key) == new_params.get(key)
            if isinstance(params_equal, np.ndarray):
                assert (expected_params.get(key) == new_params.get(key)).all()
            else:
                assert expected_params.get(key) == new_params.get(key)
...
class TestReadWriteGaussian(unittest.TestCase):
    def test_read(self):
        ...
        if _TEST_READ:
            ...
            hyperfine = reader.read(".", out_file_path, read_hyperfine=True).get_array(
                "hyperfine"
            )[-1]
